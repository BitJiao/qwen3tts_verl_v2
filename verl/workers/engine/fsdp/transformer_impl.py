...
def _qwen3_tts_finetune_forward(
    self,
    input_ids,
    codec_ids,
    ref_mels,
    ...
    codec_0_labels,
    codec_mask,
    sub_talker_loss_coef: float = 0.3,
):
...
class FSDPEngineWithQwen3TTS(FSDPEngine):
    def _build_module(self):
        ...
        with warnings.catch_warnings():
            ...
            if getattr(module, "speaker_encoder", None) is None:
                raise ValueError("Qwen3-TTS verl SFT currently requires a Base checkpoint with speaker_encoder.")

            module.forward = types.MethodType(_qwen3_tts_finetune_forward, module)
            module.to(torch_dtype)

            if self.model_config.enable_gradient_checkpointing and hasattr(module, "gradient_checkpointing_enable"):
                module.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        with autocast_ctx:
            model_output = self.module(
                input_ids=micro_batch["input_ids"],
                codec_ids=micro_batch["codec_ids"],
                ref_mels=micro_batch["ref_mels"],
                text_embedding_mask=micro_batch["text_embedding_mask"],
                codec_embedding_mask=micro_batch["codec_embedding_mask"],
                attention_mask=micro_batch["attention_mask"],
                codec_0_labels=micro_batch["codec_0_labels"],
                codec_mask=micro_batch["codec_mask"],
            )

            raw_loss = model_output["loss"]
            micro_num_tokens = micro_batch["loss_mask"].sum().to(raw_loss.device)
            batch_num_tokens = tu.get_non_tensor_data(micro_batch, key="batch_num_tokens", default=None)
            dp_size = tu.get_non_tensor_data(micro_batch, key="dp_size", default=1)
            if batch_num_tokens is None:
                loss = raw_loss
            else:
                loss = raw_loss * micro_num_tokens / max(float(batch_num_tokens), 1.0) * int(dp_size)
            ...
            output = {
                ...
                "metrics": {
                    ...
                    "codec_0_loss": model_output["codec_0_loss"].detach().item(),
                    "sub_talker_loss": model_output["sub_talker_loss"].detach().item(),
                },
            }
